...
The `MembershipManager` SHALL create draft equipment registrations for customers permitted to register equipment. This P2 interaction supersedes the older methodsketch `CreatePendingRegistration(CreatePendingRegistrationRequest): PendingRegistration` flow for draft creation; draft creation SHALL NOT directly create a backoffice task.

#### Scenario: Draft registration is created
- **WHEN** an authorized customer starts equipment registration with valid draft-registration input
- **THEN** the manager validates the customer, stores a draft or pending registration record using the equipment-registration persistence path, returns the draft registration, and records `Pending equipment registration created`
...
- **THEN** the manager returns `invalid-customer` without storing a draft registration
