...
The user enters an email address and clicks **Register**. The system receives the registration request from the client browser, validates the email format, and checks whether a user already exists for that email. If the user is already confirmed, the system returns the same accepted response as a new registration and does not disclose that an account exists. If the user is unconfirmed, the system refreshes the confirmation token and accepts the registration request. Otherwise, it creates an unconfirmed user account, creates a 24-hour email confirmation token, and returns that registration was accepted. The BPMN then continues with `SI3.1 Notify user` to send the activation email when a preliminary account is created or refreshed.

Registration is unauthenticated, so the caller must not be able to enumerate existing accounts. Valid email submissions return the same user-facing accepted response for new accounts, unconfirmed-account refreshes, and already-confirmed accounts. Already-confirmed accounts do not produce a new activation event in this interaction.
...
| 3b | Return registration accepted | Returns the generic accepted response when a confirmed account already exists, without disclosing account existence. |
...
Produced:
- `preliminaryUserCreated` after the unconfirmed account and confirmation token are created or after an unconfirmed registration token is refreshed. No event is produced for already-confirmed accounts.

## Questions / Answers

| Question | Answer |
|---|---|
| Should duplicate registration attempts be idempotent, rejected, or should they resend the activation notification? | Answered. If the account is already confirmed, return the same accepted response as other valid registration attempts and do not disclose that the account exists. If the account is still unconfirmed, treat the request as idempotent, refresh the confirmation token, and resend the activation notification. |
| How does registration prevent unauthenticated account enumeration? | Answered. Do not return `user-exists` or any distinct response for confirmed existing accounts. Use the same accepted response for all valid email submissions, and do not produce a new activation notification for confirmed existing accounts from this interaction. |
